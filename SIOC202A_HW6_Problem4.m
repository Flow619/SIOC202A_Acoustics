...
% Sound Speed Profile
c.z = z_bounds(1) + (dz/2):dz: z_bounds(2) - (dz/2); % Average z values (between z array values)
c.profile = 1500 + 0.1.*c.z; % sound speed profile
...
for i = 1:length(t)
    ...
    if c_new ~= c_old   % If moved between layers!
        ...
        theta_new = acosd( (c_new/c_old)*cosd(theta_old) );  % Snell's law, for moving between layers.
